          ...
          case BuildingType.Mosque:
            return (
              <group position={[0, 0, 0]}>
                {/* Main Body */}
                <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, 0.35, 0]} scale={[0.9, 0.7, 0.9]} />
                {/* Main Dome */}
                <mesh {...commonProps} material={new THREE.MeshStandardMaterial({ color: '#facc15' })} geometry={sphereGeo} position={[0, 0.7, 0]} scale={[0.45, 0.3, 0.45]} />
                {/* Minaret */}
                <group position={[0.35, 0, 0.35]}>
                  <mesh {...commonProps} material={mainMat} geometry={cylinderGeo} position={[0, 0.6, 0]} scale={[0.12, 1.2, 0.12]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({ color: '#facc15' })} geometry={coneGeo} position={[0, 1.3, 0]} scale={[0.15, 0.2, 0.15]} />
                </group>
                {/* Entrance Arch */}
                <mesh {...commonProps} material={accentMat} geometry={cylinderGeo} position={[0, 0.2, 0.46]} scale={[0.3, 0.4, 0.05]} rotation={[Math.PI / 2, 0, 0]} />
              </group>
            );

          case BuildingType.BigHouse:
            return (
              <group position={[0.5, 0, 0]}> {/* Shifted because it's 2 tiles wide */}
                {/* Left Wing */}
                <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[-0.45, 0.4, 0]} scale={[0.8, 0.8, 0.8]} />
                <mesh {...commonProps} material={roofMat} geometry={coneGeo} position={[-0.45, 0.9, 0]} scale={[0.7, 0.4, 0.7]} rotation={[0, Math.PI/4, 0]} />
                
                {/* Connection */}
                <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, 0.3, 0]} scale={[0.4, 0.6, 0.6]} />
                ...
                {/* Decoration */}
                <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, 0.1, 0.35]} scale={[0.3, 0.2, 0.1]} />
              </group>
            );
...
const GroundTile = React.memo(({ type, x, y, grid, onHover, onLeave, onClick }: GroundTileProps) => {
  ...
  if (type === BuildingType.None) {
    ...
    // Darken colors slightly but keep uniform height to remove boxy steps
    color = noise > 0.7 ? '#059669' : noise > 0.3 ? '#10b981' : '#34d399';
    topY = -0.3; // Uniform height for continuous feel
  } else if (type === BuildingType.Road) {
    color = '#374151';
    topY = -0.295; // slightly higher than grass to avoid z-fight but keep it close
  } else {
    color = '#cbd5e1'; // concrete base
    topY = -0.29;
  }
  ...
  return (
    ...
    >
      <boxGeometry args={[1.01, thickness, 1.01]} />
  ...
  return (
    <div className="absolute inset-0 bg-sky-900 touch-none">
      <Canvas shadows dpr={[1, 1.5]} gl={{ antialias: true }}>
        ...
        {hoveredTile && (
            <>
              <Cursor x={hoveredTile.x} y={hoveredTile.y} color={isBulldoze ? '#ef4444' : previewColor} />
              {showPreview && (
                  <group position={[previewPos[0], 0, previewPos[2]]}>
                    <ProceduralBuilding type={hoveredTool} baseColor={previewColor} x={hoveredTile.x} y={hoveredTile.y} opacity={0.5} transparent />
                  </group>
              )}
            </>
        )}
